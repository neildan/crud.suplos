...
var Property = (function Property() {
   ...
   */
  function getData() {
    const requestURL = "../crud.suplos/data-1.json";
  ...
  function createContainer(data = null, nameDiv = null) {
    const routeTemplate = "../crud.suplos/assets/template-items.html";
    const divResult = nameDiv ? $("#" + nameDiv) : $("#" + divAvailableResults);

    divResult.html("");
    $.get(routeTemplate, function (template) {
      $.tmpl(template, {
        properties: data ? data : BienesIntelcost.data,
      }).appendTo(divResult);
    });
  }
  ...
  function setFilters() {
    ...
    data.forEach((x) => {
      if (!cities.hasOwnProperty(x.Ciudad)) {
        ...
        citiesOptions +=
          "<option value='" + x.Ciudad + "'>" + x.Ciudad + "</option>";
      }
      if (!types.hasOwnProperty(x.Tipo)) {
        types[x.Tipo] = x.Tipo;
        typesOptions +=
          "<option value='" + x.Tipo + "'>" + x.Tipo + "</option>";
      }
    });
  ...
  function getSavedProperties() {
    const route = "../crud.suplos/php/property/Get.php";
   ...
   */
  function save(id) {
    const route = "../crud.suplos/php/property/Post.php";
    if (id) {
